...
def fetch_all_markets(force_refresh=False):
    ...
    try:
        ...
        # Handle case where API returns a dict with 'data' key or other structure
...
def extract_market_info(markets, force_refresh=False):
    ...
    for m in markets:
        # Debug: Check what type m is
        if not isinstance(m, dict):
            print(f"⚠ Skipping non-dict item: {type(m)}")
            continue
        
        # Use condition_question as primary text, fallback to title/question
        text = m.get('condition_question') or m.get('question') or m.get('title', '')
...
def generate_embeddings(processed_markets, force_refresh=False):
    """Generate embeddings using sentence transformers (or load from cache)"""
    ...
    print("Generating embeddings (this will take a few moments)...")
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if sim >= SIMILARITY_THRESHOLD:
                pairs.append({
                    'market_1_id': processed_markets[i]['id'],
                    'market_1_text': processed_markets[i]['original_text'],
                    'market_2_id': processed_markets[j]['id'],
...
def export_clusters(processed_markets):
    """Export clusters to CSV for manual validation"""
    
    # Create DataFrame
    df = pd.DataFrame(processed_markets)
    
    # Sort by cluster_id and similarity
    df = df.sort_values(['cluster_id', 'processed_text'])
    
    # Save full cluster details
    ...
    cluster_summary = df.groupby('cluster_id').agg({
        'id': 'count',
        'original_text': lambda x: ' | '.join(x[:3])  # First 3 examples
    }).rename(columns={'id': 'market_count', 'original_text': 'examples'})
...
def export_similar_pairs(pairs):
    """Export high-similarity pairs for review"""
    if not pairs:
...
def create_validation_template(processed_markets):
    """Create a template CSV for manual equivalence group assignment"""
    df = pd.DataFrame(processed_markets)
    
    # Add empty column for manual group assignment
    df['equivalence_group'] = ''
    df['notes'] = ''
    
    # Select relevant columns
    template = df[['id', 'original_text', 'cluster_id', 'category', 
...
def inspect_raw_data():
    """Debug helper: inspect the raw markets file"""
    print("\n" + "=" * 60)
...
def main(force_refresh=False):
    ...
    # Step 3: Generate embeddings (uses cache if available)
    ...
    cluster_sizes = pd.Series(labels).value_counts().head(10)
